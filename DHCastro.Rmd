...
text_collection =  data.frame("ID" = c(), "Year" = c(), "Month" = c(), "Day" = c(), "Iteration" = c())
year <- month <- day <- iteration <- ID <- c()
...
for(i in 1:length(file.names)){
  ...
  if (grepl(file.names[i], "-")) {
    iteration[i] <- substr(file.names[i], 14, 14)
  }
}

text_info =  data.frame("ID" = ID, "Year" = year, "Month" = month, "Day" = day, "Iteration" = iteration)
